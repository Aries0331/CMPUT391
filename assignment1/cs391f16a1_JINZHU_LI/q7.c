// prints the highest airport(s) that one can reach from YEG, 
...
int main(int argc, char *argv[]) 
{
    ...
    **************************************************************/
  	char *sql_stmt_q7 = "create table reachable as select p.* from \
                        routes r, airports p where r.Source_airport = 'YEG' \
                        and r.Destination_airport_ID = p.Airport_ID";

  	rc = sqlite3_prepare_v2(db, sql_stmt_q7, -1, &stmt, 0);
    ...
    *********************************************************/  
    ...
    /********************************************************************
    * select the one from all reachable airports with highted latitude **
    *********************************************************************/
    char *sql = "select Airport_ID, Name, City, Country, Altitude \
                from reachable order by Altitude desc limit 1";
