//  takes an airport IATA code as a parameter and 
// produces the equivalent answer of question 8 above for that airport
...
int main(int argc, char *argv[]) 
{
	...
	int rc;
  	...
  	if( argc!=3 ) {
    	fprintf(stderr, "Usage: %s <database file> <airport_IATA>\n", argv[0]);
    	return(1);
  	}
    ...
    /**************************************************************************
    * first select the airport can be reached directly from user import city **
    ***************************************************************************/
  	char *sql_stmt_q9;
  	asprintf(&sql_stmt_q9, "create table reachable as select p.* from \
                        routes r, airports p where r.Source_airport = '%s' \
                        and r.Destination_airport_ID = p.Airport_ID", argv[2]);

  	rc = sqlite3_prepare_v2(db, sql_stmt_q9, -1, &stmt, 0);
    ...
    * cannot be reached from user import airport      **
    ****************************************************/
    char *sql = "select distinct p.Airport_ID, p.Name, p.City, p.Country from airports p, \
